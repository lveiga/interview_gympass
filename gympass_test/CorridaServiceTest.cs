...
namespace gympass_test
{
    ...
    public class CorridaServiceTest
    {
        ...
        [Test]
        public void RetornaSucessoApresentarResultadoCorridaDadoListaKart()
        {
            string[] linhas = ObterTextoLogCorridaTeste().Split(new[] { Environment.NewLine }, StringSplitOptions.None);
            _kartServiceMock.Setup(x => x.ObterKartLista(linhas)).Returns(It.IsAny<Task<List<KartRacing>>>());
            _corridaServiceMock.Setup(x => x.ApresentarResultadoCorrida(It.IsAny<List<KartRacing>>())).Returns(It.IsAny<Task<List<ResultadoCorrida>>>());
            ...
            var result = upload.EnviarArquivo(mock.Object).Result;
